 ...
 * Controls all tabs that are not connected to an actual player and uses the {@link PlayerTablist} to manipulate player-connected tabs as necessary
 */
public interface SupplementaryTablist<T extends SupplementaryTablist<T>> {
    ...
    /**
     * Determines whether any code with access to the {@link Tablist} should be allowed to access the {@link PlayerTablist}
     * to make modifications to player-connected tabs.
     * @return {@code true} to permit external modification, {@code false} to forbid it
     */
    boolean allowExternalPlayerTabModification();

    /**
     * Called by {@link Tablist#applyChanges(Tablist)} if the other tablist's SupplementaryTablist is not of the same class as this one.
     * Creates and returns a SupplementaryTablist of the same class as this one at the most basic level necessary to emulate this one
    ...
    /**
     * Called by {@link Tablist#applyChanges(Tablist)}. Applies any changes made to this SupplementaryTablist to the argument.
