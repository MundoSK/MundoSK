...
import com.pie.tlatoani.TablistNew.Array.ArrayTablist;
...
public class OldTablist {
    ...
    public final PlayerTablist playerTablist = new PlayerTablist(this);
    ...
    public static final OldTablist GLOBAL = new OldTablist();
    private static final HashMap<Player, OldTablist> tablistMap = new HashMap<>();
    ...
    public static class PlayerOldTab extends OldTab.VariablyVisible {
        ...
        public PlayerOldTab(OldTablist oldTablist, Player player, boolean initialVisibility) {
            super(oldTablist, player.getName(), player.getUniqueId(), null, null, null, null, initialVisibility);
            this.player = player;
            this.icons = null;
        }
        ...
        //Not sure exactly why I'm putting the safety code here instead of TablistNew#showTab
        @Override
        public void showFor(Player target, String displayName, Byte latency, Skin icon, Integer score) {
            oldTablist.allPlayersHidden = false;
            if (oldTablist.arrayTablist.getColumns() != 4 || oldTablist.arrayTablist.getColumns() != 20) {
                oldTablist.arrayTablist.setColumns(0);
            }
        ...
        public void hideFor(Player target) {
            ...
            if (!visibleForAnyone()) {
                oldTablist.hiddenPlayers.add(player);
                oldTablist.tabs.remove(player);
            }
        }
        ...
        //The following is done because modifying the icon will change the player's own skin, so it should not be handled by TablistNew
                    ...
                    if (!event.isCancelled()) {
                        ...
                        for (PlayerInfoData playerInfoData : playerInfoDatas) {
                            ...
                            if (player != null && event.getPlayer() != null) {
                                OldTablist oldTablist = getTablistForPlayer(event.getPlayer());
                                Mundo.debug(OldTablist.class, "getTablistForPlayer = " + oldTablist);
                                OldTab oldTab = oldTablist.getTab(player);
                                Mundo.debug(OldTablist.class, "getOldTab = " + oldTab);
                                if (oldTab == null) {
                                    continue;
                                }
                                //HashMap<Player, String> tablistNames = tablist.tablistNames;
                                //Mundo.debug(TablistNew.class, "tablistNames = " + tablistNames);
                                //String tablistName = tablist.tablistNames.get(player);
                                //Mundo.debug(TablistNew.class, "tablistName = " + tablistName);
            ...
            ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(Mundo.instance, PacketType.Play.Server.NAMED_ENTITY_SPAWN) {
                ...
                public void onPacketSending(PacketEvent event) {
                    ...
                    if (tab == null || !tab.visibleFor(event.getPlayer())) {
                        Mundo.debug(OldTablist.class, "Player is hidden, event.getplayer = " + event.getPlayer().getName() + ", player = " + player.getName());
                        ...
                        UtilPacketEvent.sendPacket(playerInfoPacket(player, EnumWrappers.PlayerInfoAction.ADD_PLAYER), OldTablist.class, event.getPlayer());
                        Mundo.sync(Mundo.tablistRemoveTabDelaySpawn, () -> UtilPacketEvent.sendPacket(playerInfoPacket(player, EnumWrappers.PlayerInfoAction.REMOVE_PLAYER), OldTablist.class, event.getPlayer()));
                    }
                }
            });
            ...
            ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(Mundo.instance, PacketType.Play.Server.RESPAWN) {
                ...
                public void onPacketSending(PacketEvent event) {
                    ...
                    if (tab == null || !tab.visibleFor(player)) {
                        Mundo.debug(OldTablist.class, "Player is hidden = " + player.getName());
                        ...
                        Mundo.sync(Mundo.tablistRemoveTabDelayRespawn, new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    ...
                                    Mundo.reportException(TablistNew.class, e);
                                }
                            }
                        });*/
                        UtilPacketEvent.sendPacket(playerInfoPacket(player, EnumWrappers.PlayerInfoAction.ADD_PLAYER), OldTablist.class, player);
                        Mundo.sync(Mundo.tablistRemoveTabDelaySpawn, () -> {
                            playersRespawning.remove(player);
                            UtilPacketEvent.sendPacket(playerInfoPacket(player, EnumWrappers.PlayerInfoAction.REMOVE_PLAYER), OldTablist.class, player);
                        });
                    }
                }
            });
    ...
    public static void onJoin(Player player) {
        if (!tablistMap.containsKey(player)) {
            //setTablistForPlayer(Arrays.asList(player), new TablistNew());
            setTablistOfPlayer(player, GLOBAL);
        }
        Mundo.sync(1, () -> {
            Set<OldTablist> oldTablistSet = new HashSet<OldTablist>(tablistMap.values());
            for (OldTablist oldTablist : oldTablistSet) {
                if (oldTablist.areAllPlayersHidden()) {
                    //tablist.hidePlayers(Arrays.asList(player));
                    PlayerOldTab tab = oldTablist.getTab(player);
                    if (tab != null) {
                        tab.hideFor(null);
                    }
                }
    ...
    public static void onQuit(Player player) {
        ...
        Set<OldTablist> oldTablistSet = new HashSet<OldTablist>(tablistMap.values());
        for (OldTablist oldTablist : oldTablistSet) {
            oldTablist.hiddenPlayers.remove(player);
            oldTablist.tabs.remove(player);
        }
    }

    public static OldTablist getTablistForPlayer(Player player) {
        return tablistMap.get(player);
    }

    /*
    public static void setTablistForPlayer(Collection<Player> players, TablistNew newTablist) {
        for (Player player : players) {
            TablistNew tablist = getTablistForPlayer(player);
    ...
    public static void setTablistOfPlayer(Player player, OldTablist newOldTablist) {
        getTablistForPlayer(player).removePlayer(player);
        newOldTablist.addPlayer(player);
        tablistMap.put(player, newOldTablist);
    }
    ...
    public static void hideInTablist(Collection<? extends Player> objects, Collection<Player> subjects) {
        for (Player object : objects) {
            //PlayerInfoData playerInfoData = new PlayerInfoData(WrappedGameProfile.fromPlayer(object), 5, EnumWrappers.NativeGameMode.fromBukkit(object.getGameMode()), WrappedChatComponent.fromJson(TablistNew.colorStringToJson(object.getPlayerListName())));
