...
public class UtilRegistration {
    ...
    public static <T> ClassInfo<T> registerType(Class<T> type, String name, String... alternateNames) {
        ...
        names.add(0, name);
        ClassInfo<T> result = new ClassInfo<T>(type, name).user(names.toArray(new String[0])).name(name).parser(new Mundo.SimpleParser<T>() {
            @Override
            public T parse(String s, ParseContext parseContext) {
                return null;
            }
        });
        if (Mundo.classInfoSafe(type, name)) {
    ...
    public static <E> void registerEnum(Class<E> enumClass, String name, E[] values, Map.Entry<String, E>... defaultPairings) {
        if (!Mundo.classInfoSafe(enumClass, name)) return;
