...
public class ExprRandomIndex extends SimpleExpression<String> {
	...
	protected String[] get(Event e) {
		if (numbers instanceof Variable) {
			...
			List<Object> probs = Arrays.asList(numbers.getAll(e));
			List<Number> nums = new ArrayList<Number>();
			Number sum = 0;
			for (int i = 0; i < probs.size(); i++) {
				sum = sum.doubleValue() + ((Number) probs.get(i)).doubleValue();
				nums.add(sum);
			}
			Number random = Math.random() * sum.doubleValue();
			Boolean searching = true;
			...
			return new String[]{(new Integer(j + 1)).toString()};
		}
	}
